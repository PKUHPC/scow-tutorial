...
# 
# Tutorial系列的这部分旨在展示更接近实际的训练场景，使用ResNet50训练ImageNet数据集，在多块V100显卡上做并行。
# 
# 分以下几步来实现：
# 
# 1. 数据加载和预处理
...
# 训练函数
import time
def train(model, train_loader, loss, optimizer, devices, logger):
    model.train()
    ...
    for X, y in train_loader:
        ...
        if (count > 1) and (count % 1000 == 1):

            average_loop_time = sum(time_loop[-1000:]) / 1000
            average_train_time = sum(time_train.times[-1000:]) / 1000
            average_loss = sum(loss_list[-1000:]) / 1000
            
            logger.log(f'''batch: {count} | batch loss: {average_loss} | loop_time: {average_loop_time} | train_time: {average_train_time}
                       {'-'*60}
                       ''')
            
        time_loop.append(time.time()-tik1)
        tik1 = time.time()
        
    average_loss = sum(loss_list) / len(loss_list)

    return average_loss, sum(time_loop), time_train.sum()
...
class log_process:
    """在训练过程中动态绘制数据"""
    def __init__(self, epochs, figsize=(4, 3), save_path='log_process.png'):
        backend_inline.set_matplotlib_formats('svg')
        self.epochs = epochs
        self.fig, self.ax1 = plt.subplots(figsize=figsize)
        self.ax2 = self.ax1.twinx()
        self.epochs = epochs
        self.save_path = save_path  # 保存图像的路径
        self.train_ls = []
        self.test_acc = []
...
parser.add_argument('-logfile', type=str, required=True, help='Name of the log file')
args = parser.parse_args()
logger = Logger(args.logfile)

# 参数设置
epochs, batch_size, gpu_n, num_workers = 20, 256, 2, 8
learning_rate, momentum, weight_decay = 0.1, 0.9, 1e-4
topk = (1, 5)
show_top_k = 5

logger.log(f''' epochs, batch_size, gpu_n, num_workers = {epochs}, {batch_size}, {gpu_n}, {num_workers}
learning_rate, momentum, weight_decay = {learning_rate}, {momentum}, {weight_decay}
topk, show_top_k = {topk}, {show_top_k}''')

# 查看 GPU 数量
gpu_total = torch.cuda.device_count()
if gpu_total < gpu_n or gpu_total < 1:
    logger.log(f"ValueError: Value gpu_n should <= {gpu_total} and gpu_total should > 0.")
...
load_time = []
val_time = log_time()
save_path = os.path.join('/'.join(args.logfile.split('/')[:-1]), 'output.png')
train_process = log_process(epochs, figsize=(4, 3), save_path='log_process.png')
for epoch in range(epochs):
    
    # 训练
    batch_average_loss, epoch_load_time, epoch_train_time = train(model, train_loader, loss, optimizer, devices, logger)
    ...
    logger.log(f'''epoch: {epoch}
               loss: {average_loss} | test accuracy: {accuracy_dict} | 
               load_time: {average_load_time} | train_time: {average_train_time} |  val_time: {average_val_time}
               {'='*100}
               ''')
