...
# ## 3. 训练与评估
# 
# 我们需要在多卡上并行训练，这里我们使用的是四张V100显卡。
# 
# 在训练过程中我们增加了学习率调整策略，以加速收敛。
# 
# 完成模型训练和评估在 V100 显卡上需要约 ***
# 
...
def train(model, train_loader, loss, optimizer, devices, logger):
    ...
    while True:
        ...
        if (count > 1) and (count % 1000 == 1):
            ...
            average_time_all = sum(time_all.times[-1000:]) / 1000
            average_load_all = sum(time_load_all.times[-1000:]) / 1000
            average_print = sum(time_print.times[-1000:]) / 1000
            
            logger.log(f'''batch: {count} | batch loss: {average_loss} |
                       average_time_all: {average_time_all} | average_load_all: {average_load_all} | average_load_time: {average_load_time} |
                       average_train_time: {average_train_time} | average_print: {average_print} 
                       {'-'*60}
                       ''')
